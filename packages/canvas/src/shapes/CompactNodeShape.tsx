 ...
 * have a dedicated custom shape.  Renders as a compact rounded badge
 * with subsystem-colored dot, label, and typed ports derived from the
 * node's NodeDef.
 *
 * Used for: sys:env, sys:disk, sys:network, data:filter, data:merge,
 * data:fetch, view:metric, view:chart, and many more.
 */
...
export type CompactNodeShape = TLBaseShape<
  "compact-node",
  {
    ...
    h: number;
    label: string;
    nodeType: string;     // "sys:disk", "data:filter", etc.
    subsystem: string;    // "system", "data", "view"
    configJson: string;   // JSON-stringified config
  }
>;
...
export class CompactNodeShapeUtil extends ShapeUtil<CompactNodeShape> {
  ...
  getDefaultProps(): CompactNodeShape["props"] {
    return {
      ...
      h: 48,
      label: "",
      nodeType: "",       // empty = blank node, user picks type
      subsystem: "",
      configJson: "{}",
    };
  }
...
// Node type descriptions for the picker
...
function CompactComponent({ shape }: { shape: CompactNodeShape }) {
  ...
  // Capture-phase for picker input
  ...
  // Look up NodeDef for port definitions
  const nodeDef = runtime?.getNodeDef(nodeType);
  const inputs = nodeDef?.inputs ?? [];
  const outputs = nodeDef?.outputs ?? [];

  // Derive status from runtime node state
  let status: "idle" | "running" | "done" | "error" = "idle";
  if (runtime) {
    const runtimeNodeId = getNodeId(shape.id);
    if (runtimeNodeId) {
      const nodeState = runtime.graph.getNode(runtimeNodeId);
      if (nodeState) {
  ...
  // Short type label for display (e.g. "sys:disk" -> "disk")
      ...
      >
        ...
        {inputs.map((inp, i) => (
          <Port
            key={`in-${inp.name}`}
            side="left"
            type={(inp.type === "data" || inp.type === "text" || inp.type === "number" || inp.type === "image" || inp.type === "any") ? inp.type as PortType : "any"}
            name={inp.name}
            shapeId={shape.id}
            index={i}
            total={inputs.length}
        ...
        {outputs.map((out, i) => (
          <Port
            key={`out-${out.name}`}
            side="right"
            type={(out.type === "data" || out.type === "text" || out.type === "number" || out.type === "image" || out.type === "any") ? out.type as PortType : "any"}
            name={out.name}
            shapeId={shape.id}
            index={i}
        ...
        {/* Fallback ports when no NodeDef */}
        {inputs.length === 0 && outputs.length === 0 && !isBlank && (
          <>
